...
from config.conf import (get_broker_and_backend, get_redis_master)
...
if isinstance(broker_and_backend, list):
    ...
    app.conf.update(BROKER_TRANSPORT_OPTIONS = {'master_name': master})

app.conf.update(
    CELERY_TIMEZONE = 'Asia/Shanghai',
    CELERY_ENABLE_CUT = True,
    ...
    CELERYBEAT_SCHEDULE={
        ...
        'home_task': {
            'task': 'task.home.execute_home_task',
    ...
    CELERY_QUEUES=(
        ...
        Queue('comment_page_crawler', exchange=Exchange('comment_page_crawler', type='direct'), routing_key='comment_page_info'),
        Queue('repost_crawler', exchange=Exchange('repost_crawler', type='direct'), routing_key='repost_page_info'),
    ),
)
