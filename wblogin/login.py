# -*- coding=UTF-8 -*-
import re,os,rsa,math,time,random,base64,binascii
...
def login_by_pincode(name, password, session, server_data, retry_count):
    ...
    if not yundama_username:
        raise Exception('login need verification code, please set you yundama info in config/spider.yaml')
    img_url = get_pincode_url(pcid)
    pincode_name = get_img(img_url, name, retry_count)
    verify_code, yundama_obj, cid = code_verification.code_verificate(yundama_username, yundama_password, pincode_name)
...
def get_session(name, password):
    ...
    other.error('login failed for user: {}' . format(name))
