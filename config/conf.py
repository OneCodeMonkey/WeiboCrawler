...
def get_broker_and_backend():
    ...
    setinel_args = redis_info.get('sentinel', '')
    db = redis_info.get('broker', 5)
    if setinel_args:
        broken_url = ";" . join('sentinel://:{}@{}:{}/{}' . format(password, sentinel['host'], sentinel['port'], db) for sentinel in setinel_args)
        return broken_url
    else:
        host = redis_info.get('host')
        port = redis_info.get('port')
        backend_db = redis_info.get('backend', 6)
        broken_url = 'redis://:{}@{}:{}/{}' . format(password, host, port, db)
        backend_url = 'redis://:{}@{}:{}/{}' . format(password, host, port, backend_db)
        return broken_url, backend_url
