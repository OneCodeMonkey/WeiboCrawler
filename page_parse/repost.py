...
def get_total_page(html):
    try:
        ...
        parser.error('Errors occured when parsing total page of repost, specification is {}'.format(e))
        page_count = 1
...
def get_repost_list(html, mid):
    ...
    for repost in reposts:
        ...
        try:
            repost_cont = repost.find(attrs={'class': 'WB_text'}).find(attrs={'node-type': 'text'}).text.strip().split('//@')
            wb_repost.repost_cont = repost_cont[0].encode('GBK', 'ignore').decode('GBK', 'ignore')
            wb_repost.weibo_id = repost['mid']
            # TODO 将 wb_repost.user_id 加入待爬队列（seed_ids）
            ...
            wb_repost.weibo_url = repost_url.format(repost.find(attrs={'class': 'WB_from S_txt2'}).find('a').get('href'))
            ...
            if not parents:
                ...
                try:
                    ...
                    parser.error("error occured when parsing the parent's name, the detail is {}".format(e))
                    wb_repost.parent_user_name = ''

        except Exception as e:
            parser.error('report parse error occured, the detail is {}'.format(e))
        else:
